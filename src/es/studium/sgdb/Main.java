...
public class Main extends Connections implements WindowListener, ActionListener {
	...
	// ==================================================== FUNCIONES
	// ====================================================
	public Main(int privilegio, String username) {
	...
	/**
	 * itera un Array de objetos AttributeButtons para comprobar que el botón pasado
	 * como parámetro fue generado anonimamente como botón de Foreign Key de la
	 * casilla de una fila.
	 * 
	 * @param button
	 * @return String vacio si no es un boton de FK, nombre de la columna en la que
	 *         está si sí es un FK
	 */
	...
	/**
	 * Construye los botones de las columnas de la tabla. Almacena el ancho de cada
	 * una. Almacena cual columna es FK. Añade las columnas al Choice del Navbar si
	 * se ha cambiado de tabla.
	 * 
	 */
	private void build_table_columns() {
		...
		for (int x = 0; x < table_columns_ammount; x++) {
			...
			if (changed_table)
				ch_searchcolumn.addItem(new_columnname);

			// dar un ancho a esta columna concreta
			column_size = 120;
			if (new_columnname.equals("DOMICILIO") || new_columnname.equals("MAIL") || new_columnname.equals("REQUERIMIENTO"))
				column_size = 250;
			if (new_columnname.equals("ID"))
				column_size = 50;
	...
	/**
	 * Construye las filas de las tablas. Según los filtros textuales y de orden
	 * Según el número de filas que se muestren (11/20 de 230) Si el campo a
	 * construir es FK, pone un botón y no un Label Al final de cada fila coloca un
	 * boton de BORRAR y de ACTUALIZAR
	 */
	private void build_table_rows() {
		...
		try {
			...
			new Log(username, "SELECT", sentence.trim());
				...
				if (rs.getRow() >= first_row_limit && rs.getRow() <= last_row_limit) {
					...
					for (int x = 0; x < table_columns_ammount; x++) {
						...
						if (current_columndata != null) {
							if (tables[index_tables_list][x].indexOf("sueldo") != -1)
								current_columndata += "€";
						}
						...
						if (!current_gap_fk) {
							...
							// obtener el de esta fila, que es el primer campo, para los botones de
							// ACTUALIZAR Y BORRAR.
							if (current_row_id.length() == 0)
								current_row_id = current_columndata;
					...
					for (int y = 0; y < list_delete_buttons.length; y++) {
						if (list_delete_buttons[y] == null) {
							list_delete_buttons[y] = new AttributeButton("", table_name, table_incolumn_name,
									current_row_id, btn_delete_row);
					...
					for (int y = 0; y < list_update_buttons.length; y++) {
						if (list_update_buttons[y] == null) {
							list_update_buttons[y] = new AttributeButton("", table_name, table_incolumn_name,
									current_row_id, btn_update_row);
	...
	/**
	 * Función que decora a build_table_rows() y build_table_columns(). Borra el
	 * choice del Navbar y el interiore de la tabla Redimensiona el Frame.
	 */
	private void load_table() {
		...
		if (changed_table)
			lbl_title.setText(table_name.toUpperCase().replace("_", " "));
	...
	/**
	 * Para diferenciar entre si ha cargado la tabla, por un filtro de texto o de
	 * orden, o si ha cambiado de tabla. Asigna variables de la tabla actual.
	 * 
	 * @param table_name
	...
	private void first_table_load(String table_name, String table_incolumn_name, int index_columns_names) {
		...
		// Borra el contenido del TextField de  busqueda de palabras
		tf_selectrow.setText("");

		// borrar el select de las columnas
		sentencia.emptyAll();
	...
	/**
	 * Ejecutado cuando se hace click en un botón de FK. Rellena los datos de la
	 * fila referida del FK.
	 * 
	...
	private void show_dataFK_dialog(String sentence, int index) {
		...
		new Log(username,  "FOREIGN KEY", sentence.trim());
						...
						if (str_ends_in(column_name_dialog, "FK")) { // =================== SI ES FK ===================
							...
							for (int y = 0; y < list_FK_buttons.length; y++) {
								if (list_FK_buttons[y] == null) {
									...
									list_FK_buttons[y] = new AttributeButton(column_name_dialog, "", "", "",
											btn_column_data_dialog);
	...
	private void search_filter(boolean exact) {
		...
		if (!str_ends_in(sentencia.getFilterColumnName(), "FK")) {
			...
			// Le añadimos el nombre de la tabla: de "ID" --> "id" --> "idCliente"
		...
		if (tf_selectrow.getText().length() != 0) {
			...
			if (exact) {
				if (cb_searchrow_invert.getState()) {
					sentencia.setFilter("WHERE " + sentencia.getFilterColumnName() + " NOT LIKE '" + tf_value + "'");
				} else {
					sentencia.setFilter("WHERE " + sentencia.getFilterColumnName() + " LIKE '" + tf_value + "'");
				}
			} else {
				if (cb_searchrow_invert.getState()) {
					sentencia.setFilter("WHERE " + sentencia.getFilterColumnName() + " NOT LIKE '%" + tf_value + "%'");
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (e.getSource().equals(move_left_row)) // ===================== MOVER IZQUIERDA =====================
		{
			...
			new PDF(this.table_name, this.index_tables_list);
		...
		} else if (e.getSource().equals(mniSalir)) {
			System.exit(0);
		} // ================================ ALTAS ================================
				...
				// ============================================
		{
			...
			if (current_btn_str.equals(" X ")) {
				...
				for (int x = 0; x < list_delete_buttons.length; x++) {
					...
					if (list_delete_buttons[x] != null) {
						...
						if (e.getSource().equals(list_delete_buttons[x].getButton())) {
							...
							new Baja(current_table_column_name, current_table_name, tables[index_tables_list],
									current_row_id);
							// salimos del bucle
				...
				for (int x = 0; x < list_update_buttons.length; x++) {
					...
					if (list_update_buttons[x] != null) {
						...
						if (e.getSource().equals(list_update_buttons[x].getButton())) {
							...
							String current_table_name = list_delete_buttons[x].getTable_name();
							String current_row_id = list_delete_buttons[x].getId();
							new Modificacion(current_table_column_name, current_table_name, tables[index_tables_list],
									current_row_id);
							// salimos del bucle
							x = list_update_buttons.length;
						}
				...
				if (fkcolumn_name.length() != 0) {
					...
					case "idClienteFK":
						fkcolumn_name = "clientes";
						fkcolumn_intable_name = "Cliente";
						index_table = 0;
						break;

					case "idJefeFK":
						fkcolumn_name = "empleados";
					...
					// Construir sentencia
					String sentence = "SELECT * FROM " + fkcolumn_name + " WHERE id" + fkcolumn_intable_name + " = "
							+ current_btn_str;
